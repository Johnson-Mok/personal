...
def draw_key(img, key, pos, key_w, key_h, letter_w, letter_h):
    ...
    cv2.rectangle(img, startpoint, endpoint,
                  colour_border, thick_border)  # Border
...
def press_after_x_seconds(img, letter, wait_sec, new_run, start_time, letter_start):
    ...
    if (new_run):  # Start counting if condition satisfied
        ...
        new_run = False
    # if more than 2 seconds, write
    elif ((time.time()-start_time >= wait_sec) & (letter_start == letter) & ~new_run):
        print(letter)
        pg.press(letter)  # Actual write, open a text document to see output
        new_run = True
    # if less than 2 seconds, wait
    elif ((time.time()-start_time < wait_sec) & (letter_start == letter) & ~new_run):
        cv2.putText(img, f'{time.time()-start_time:.2f}', (10, 70),
                    cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 255), 3)  # Show hold timer
