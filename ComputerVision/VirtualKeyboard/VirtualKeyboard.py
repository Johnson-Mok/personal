'''
Summary:
        Virtual keyboard making use of computer vision, OpenCV, and 
        handtracking via mediapipe.
Sources:
        Inspiration: https://faun.pub/virtual-keyboard-using-computer-vision-32b61981c5b1 # noqa
...
if __name__ == "__main__":
    ...
    while True:
        ...
        draw_virtual_keyboard(
            img, key_w, key_h, button_pos_list, letter_w, letter_h)

        # Show caps lock status
        CAPSLOCK = CAPSLOCK_STATE()
        if ((CAPSLOCK) & 0xffff) != 0:
            cv2.putText(img, "Lower", (int(w*0.8), 70),
                        cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 3)
        elif ((CAPSLOCK) & 0xffff) == 0:
            cv2.putText(img, "Upper", (int(w*0.8), 70),
                        cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 3)
                ...
                for id, lm in enumerate(hand_lms.landmark):
                    if id == index_finger_tip:  # Annotate part of hand
                        ...
                        for pos, letter in button_pos_list:
                            # Index finger within a key box
                            condition = (
                                pos[0] < cx < pos[0] + key_w) & (pos[1] < cy < pos[1] + key_h)
                            if condition:
                                new_run, start_time, letter_start = press_after_x_seconds(
                                    img, letter, 2, new_run, start_time, letter_start)  # Hold for 2 seconds to write
        ...
        if cv2.waitKey(1) & 0xFF == 27:  # escape is set as the quitting button
            print('break')
            if ((CAPSLOCK) & 0xffff) != 0:
                pg.press("capslock")  # Turn caps lock of
                # when shutting down.
